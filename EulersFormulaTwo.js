...
// init variables
var currency, currency_T, currency_B;
...
var init = () => {
    ...
    currency_I = theory.createCurrency("B", "B");
    ...
    // b1
    {
        ...
        b1 = theory.createUpgrade(3, currency_T, new FirstFreeCost(ExponentialCost(20, Math.log2(200))));
    ...
    // b2
    {
        ...
        b2 = theory.createUpgrade(4, currency_T, new ExponentialCost(100, Math.log2(2)));
    ...
    // c1
    {
        ...
        c1 = theory.createUpgrade(5, currency_B, new FirstFreeCost(new ExponentialCost(20, Math.log2(200))));
    ...
    // c2
    {
        ...
        c2 = theory.createUpgrade(6, currency_B, new ExponentialCost(100, Math.log2(2)));
    ...
    // a2
    {
        ...
        a2 = theory.createUpgrade(8, currency_T, new ExponentialCost(500, 2.2));
    ...
    // a3
    {
        ...
        a3 = theory.createUpgrade(9, currency_B, new ExponentialCost(500, 2.2));
    ...
    // t1
    {
        let getDesc = (level) => "t_1=2^{" + level + "}";
        let getInfo = (level) => "t_1=" + getQ2(level).toString(0);
        t1 = theory.createUpgrade(9, currency_B, new ExponentialCost(500, 2.2));
    ...
    // t2
    {
        let getDesc = (level) => "t_2=2^{" + level + "}";
        let getInfo = (level) => "t_2=" + getQ2(level).toString(0);
        t2 = theory.createUpgrade(9, currency_B, new ExponentialCost(500, 2.2));
        t2.getDescription = (_) => Utils.getMath(getDesc(t1.level));
        t2.getInfo = (amount) => Utils.getMathTo(getInfo(t2.level), getInfo(t2.level + amount));
    }
...

    {
        ...
        dimension.getDescription = () => dimension.level == 0 ? "Unlock the terza component T" : "Unlock the beta component B";
...
var checkForScale = () => {
    if(max_T > 1.5 / scale || max_B > 1.5 / scale) { // scale down everytime T or B gets larger than the screen
        theory.clearGraph();
        t_graph = BigNumber.ZERO;
        state.x = t_graph.toNumber();
        state.y = T.toNumber();
...
var tick = (elapsedTime, multiplier) => {
    ...
    // these T and B values are used for coordinates on the graph
    T = b * t.cos(); // b * cos(t) - terza part of solution
    B = c * t.sin(); // c * i * sin(t) - "beta" part of solution
    max_T = max_T.max(T);
    max_B = max_B.max(B);
    ...
    if(q1.level == 0) {
        ...
        currency_T.value = BigNumber.ZERO;
        currency_B.value = BigNumber.ZERO;
    } else {
        // rho calculation
        switch (dimension.level) {
            case 0:
                ...
                break;
            case 1:
                currency.value += base_currency_multiplier * (t * q.pow(BigNumber.TWO) + (currency_T.value).pow(BigNumber.TWO)).sqrt();
                break;
            case 2:
                currency.value += base_currency_multiplier * a * (t * q.pow(BigNumber.TWO) + (currency_T.value).pow(BigNumber.TWO) + (currency_B.value).pow(BigNumber.TWO)).sqrt();
                break;
        }

        // T calculation
        currency_T.value += dimension.level > 0 ? base_currency_multiplier * T.square() : BigNumber.ZERO;

        // B calculation
        currency_B.value += dimension.level > 1 ? base_currency_multiplier * B.square() : BigNumber.ZERO;
...
var getQuaternaryEntries = () => {
    ...
    quaternaryEntries[3].value = B.toString(2) + "i";
...
var getT1 = (level) => BigNumber.TWO.pow(level);
var getT2 = (level) => BigNumber.TWO.pow(level);
